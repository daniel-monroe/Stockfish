...
moves_loop:  // When in check, search starts here
    ...
    Value specBestValue = -VALUE_INFINITE;
    Move  specBestMove  = Move::none();
    if (depth >= 10)
    {
        ...
        while ((move = spec_mp.next_move()) != Move::none())
        {
            ...
            if (move == excludedMove || move == ttData.move)
                continue;
            ...
            Value nextValue = ttHitSpec
                              ? value_from_tt(ttDataSpec.value, ss->ply + 1, pos.rule50_count())
                              : VALUE_NONE;
