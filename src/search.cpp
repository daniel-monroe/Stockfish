...
static int improvingDeductions[5] = {59, 59, 59, 59, 59};
static int notImprovingDeduction  = 0;

TUNE(improvingDeductions);
TUNE(SetRange(-50, 50), notImprovingDeduction);
...
Value futility_margin(Depth d, bool noTtCutNode, int evalDiff, bool oppWorsening) {
    ...
    int   idm;
    if (evalDiff > 0)
        idm = improvingDeductions[std::clamp(evalDiff / 50, 0, 4)];
    else
        idm = notImprovingDeduction;
