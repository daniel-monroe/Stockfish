    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth / 1024 < 7 && !ss->inCheck)
                {
                    Value futilityValue = ss->staticEval + 287 + 253 * lmrDepth / 1024
            ...
            else
            {
                ...
                lmrDepth += history * 1024 / 3609;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 45 ? 140 : 43) + 141 * lmrDepth / 1024;

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth / 1024 < 12 && futilityValue <= alpha)
                {
