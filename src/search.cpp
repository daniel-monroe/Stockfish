...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && !excludedMove && ttData.depth > depth - (ttData.value <= beta) - ((ss-1)->currentMove == Move::null())
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - 20 * depth + 470 - 60 * improving - 200 * (depth >= 15) && !excludedMove
